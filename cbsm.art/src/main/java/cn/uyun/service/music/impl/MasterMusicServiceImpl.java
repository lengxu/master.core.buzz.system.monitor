...
@Component
@Profile("master")
public class MasterMusicServiceImpl implements MusicService {

    private static final Logger logger = LoggerFactory.getLogger(MasterMusicServiceImpl.class);
    ...
    //music编码
    private static HashMap<String, HashMap<String, String>> musicCodes = new HashMap();
    ...
    public MasterMusicServiceImpl(){
        ...
        dataType.put("NAFP_CLDAS2.0_RT_GRB", "小时地面要素");
        dataType.put("NAFP_CLDAS2.0_RT_5KM_DAY_GRB", "日地面要素");
        dataType.put("SURF_CMPA_FRT_5KM", "小时降水（实时）");
        dataType.put("SURF_CMPA_FRT_5KM_10MIN", "10分钟降水");
        dataType.put("SURF_CMPA_FRT_5KM_DAY", "日降水（实时）");
        dataType.put("SURF_CMPA_FAST_5KM", "小时降水（快速）");
        dataType.put("SURF_CMPA_FAST_5KM_DAY", "日降水（快速）");
        dataType.put("NAFP_3DCloudA_GRIB2", "小时三维要素");
        dataType.put("OCEN_PRODUCT_PART1_FILE", "全球海表温度");
    }
    ...
    public Object return_byte_size_query_time() {
        ...
        PointsQueryBean pqb2 = new PointsQueryBean();
        ...
        groupBy.put("tag_keys",new String[]{"api_data_code"});
        pqb.setGroup_by(groupBy);
        pqb.setTime(map);
        pqb2.setGroup_by(groupBy);
        pqb2.setTime(map);

        List<Map<String,Object>> query_time = new ArrayList<Map<String, Object>>();
        List<Map<String,Object>> return_byte_size = new ArrayList<Map<String, Object>>();
        //---------------数据的字节数,1.1.3 调用次数 NAFP_CLDAS2.0_RT_GRB------
        for (Map.Entry<String, String> entry: dataType.entrySet()) {
            Map<String, Object> tags = new HashMap<String, Object>();
            tags.put("api_data_code", entry.getKey());
            tags.put("return_code","0");
            pqb.setTags(tags);
            pqb.setMetric("M.CIMISS.MUSIC.QUERY_TIME");
            if(resultData(tags,pqb)) {
                tags.put("api_data_code", entry.getValue());
                query_time.add(tags);
            }
            ...
            tags.put("api_data_code", entry.getKey());
            pqb2.setTags(tags);
            pqb2.setMetric("M.CIMISS.MUSIC.RETURN_BYTE_SIZE");
            if(resultData(tags,pqb2)){
                tags.put("api_data_code", entry.getValue());
                return_byte_size.add(tags);
            }
        }
        outMap.put("query_time",query_time);
        outMap.put("return_byte_size",return_byte_size);
        return outMap;
    }
